...
class ShellEngine {
  ...
  factory ShellEngine.seeded() {
    return ShellEngine._({
      '/home/hero': ['notes.txt', '.clue', 'deploy.sh', 'logs', 'vault'],
  ...
  ShellResult run(String rawInput) {
    ...
    return switch (command) {
      ...
      'mv' => _move(args),
      'cat' => _cat(args),
      'nano' => const ShellResult(output: 'nano opened in training mode. File saved on exit.'),
      'chmod' => _chmod(args),
      'grep' => _grep(args),
      'ps' => const ShellResult(output: 'USER PID %CPU COMMAND\nhero 101 0.1 bash\nroot 318 2.4 backupd\nhero 404 0.7 dart'),
      'kill' => _kill(args),
      'ping' => _ping(args),
      'curl' => _curl(args),
      'ssh' => _ssh(args),
  ...
  ShellResult _ls(List<String> args) {
    ...
    final visible = entries.where((entry) => includeHidden || !entry.startsWith('.'));
    final output = long
        ? visible.map((entry) => '-rw-r--r-- hero hero  ${entry.padRight(12)}').join('\n')
        : visible.join('  ');
  ...
  ShellResult _cd(List<String> args) {
    ...
    return ShellResult(output: 'cd: no such directory: ${args.first}', path: _cwd, success: false);
  }

  ShellResult _mkdir(List<String> args) {
    if (args.isEmpty) return const ShellResult(output: 'mkdir: missing operand', success: false);
    final target = _resolve(args.first);
    _files[target] = [];
    _files[_cwd] = {...?_files[_cwd], args.first}.toList();
    return ShellResult(output: 'created directory ${args.first}', path: _cwd);
  }

  ShellResult _touch(List<String> args) {
    if (args.isEmpty) return const ShellResult(output: 'touch: missing file operand', success: false);
    _files[_cwd] = {...?_files[_cwd], args.first}.toList();
    return ShellResult(output: 'created file ${args.first}', path: _cwd);
  }

  ShellResult _remove(List<String> args) {
    if (args.isEmpty) return const ShellResult(output: 'rm: missing operand', success: false);
    _files[_cwd]?.remove(args.last);
    return ShellResult(output: 'removed ${args.last}', path: _cwd);
  }

  ShellResult _copy(List<String> args) {
    if (args.length < 2) return const ShellResult(output: 'cp: missing source or destination', success: false);
  ...
  ShellResult _move(List<String> args) {
    if (args.length < 2) return const ShellResult(output: 'mv: missing source or destination', success: false);
  ...
  ShellResult _cat(List<String> args) {
    if (args.isEmpty) return const ShellResult(output: 'cat: missing file operand', success: false);
    ...
    return ShellResult(output: output, path: _cwd, success: !output.contains('No such file'));
  }

  ShellResult _chmod(List<String> args) {
    if (args.length < 2) return const ShellResult(output: 'chmod: missing mode or file', success: false);
    return ShellResult(output: 'mode of ${args[1]} changed to ${args[0]}', path: _cwd);
  }

  ShellResult _grep(List<String> args) {
    if (args.length < 2) return const ShellResult(output: 'grep: usage grep PATTERN FILE', success: false);
    return ShellResult(output: '${args[1]}: matched "${args[0]}" on line 1', path: _cwd);
  }

  ShellResult _kill(List<String> args) {
    if (args.isEmpty) return const ShellResult(output: 'kill: missing process id', success: false);
  ...
  ShellResult _ping(List<String> args) {
    ...
    return ShellResult(output: 'PING $host: 56 data bytes\n64 bytes from $host: ttl=64 time=8.2 ms', path: _cwd);
  }

  ShellResult _curl(List<String> args) {
    final url = args.isEmpty ? 'https://terminal.hero' : args.first;
    return ShellResult(output: 'HTTP/2 200\n{"status":"training-online","url":"$url"}', path: _cwd);
  }

  ShellResult _ssh(List<String> args) {
    if (args.isEmpty) return const ShellResult(output: 'ssh: missing destination', success: false);
    return ShellResult(output: 'Connected to ${args.first}. Training sandbox active.', path: _cwd);
  }

  ShellResult _git(List<String> args) {
    if (args.isNotEmpty && args.first == 'status') {
      return ShellResult(output: 'On branch main\nnothing to commit, working tree clean', path: _cwd);
    }
    return const ShellResult(output: 'git: try git status', success: false, suggestion: 'git status');
  }
  ...
  String? _suggest(String command) {
    const known = ['ls', 'cd', 'pwd', 'mkdir', 'touch', 'cat', 'chmod', 'grep', 'ps', 'ping', 'ssh'];
